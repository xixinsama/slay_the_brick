...
# 界面引用
@export var card_display_ui: Control     # 卡牌展示界面预制体
@export var card_prefab: PackedScene     # 卡牌预制体
...
func _ready() -> void:
	...
	# 初始化工作列表
	working_list = card_list.duplicate()
...
func _on_pile_clicked() -> void:
	if card_display_ui:
		# 显示卡牌展示界面
		var display = card_display_ui.instantiate()
		get_tree().current_scene.add_child(display)
		
		# 传递数据并初始化
		display.init_display({
			"cards": working_list.duplicate(),
			"title": text,
			"allow_sort": true
		})
		
		# 连接关闭信号
		display.closed.connect(func(): 
			display.queue_free()
		)
